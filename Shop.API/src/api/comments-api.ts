...
commentsRouter.post(
  ...
  async (req: Request<{}, {}, CommentCreatePayload>, res: Response) => {
    try {
      ...
      const [info] = await connection.query<ResultSetHeader>(postComment, [
        id,
        email,
        name,
        body,
...
commentsRouter.patch(
  ...
  async (req: Request<{}, {}, Partial<IComment>>, res: Response) => {
    try {
      ...
      const id = uuidv4();
      await connection.query<ResultSetHeader>(postComment, [
        id,
        newComment.email,
        newComment.name,
        newComment.body,
...
commentsRouter.delete(
  ...
  async (req: Request<{ id: string }>, res: Response) => {
    try {
      const [info] = await connection.query<ResultSetHeader>(delleteComment, [
        req.params.id,
      ]);
