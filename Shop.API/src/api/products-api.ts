...
import {
  ...
  postImages,
  getCurrentCover,
  getAvailabilityOfNewCover,
  resetCurrentCover,
  setNewCover,
  setDataUpdate,
  getSimilarProducts,
  postSimilarProducts,
  delleteSimilarProducts,
  delleteAllSimilarProducts,
...
productsRouter.post(
  ...
  async (
    ...
    try {
        ...
        for (let i = 0; i < resultImages.length; i++) {
          try {
            const [postImageResult] = await connection.query<ResultSetHeader>(
              postImages,
              [
                resultImages[i].image_id,
                resultImages[i].url,
                resultImages[i].product_id,
                resultImages[i].main,
              ]
            );
      ...
      await connection.query<ResultSetHeader>(postProduct, [
        id,
        title || null,
        description || null,
        price || null,
...
productsRouter.delete(
  ...
  async (req: Request<{ id: string }>, res: Response) => {
    try {
      ...
      if (
        infoImages.affectedRows === 0 &&
        infoComments.affectedRows === 0 &&
        resultsDelleteSilimar.affectedRows === 0 &&
        infoProduct.affectedRows === 0
      ) {
        res.status(404);
        res.send(`Product with id ${req.params.id} is not found`);
        return;
...
productsRouter.post(
  "/delete_images",
  async (req: Request<{}, {}, IdeleteImages>, res: Response) => {
    try {
      ...
      const [deleteImg] = await connection.query<ResultSetHeader>(
        delleteImage,
        [[imagesToRemove]]
      );
...
productsRouter.post(
  "/set_images",
  async (req: Request<{}, {}, IsetImages>, res: Response) => {
    try {
      ...
      res
        .status(201)
        .send(
          `images to the product ${product_id} have been successfully added`
        );
    } catch (error) {
...
productsRouter.post(
  ...
  async (req: Request<{ id: string; idChange: string }>, res: Response) => {
    try {
      ...
      try {
        await connection.query<ResultSetHeader>(resetCurrentCover, [
          idChangeProduct,
        ]);

        await connection.query<ResultSetHeader>(setNewCover, [
          idChange,
          idChangeProduct,
        ]);
...
productsRouter.patch(
  ...
  async (
    ...
    try {
      ...
      try {
        await connection.query<ResultSetHeader>(setDataUpdate, [
          title,
          description,
          price,
          id,
    ...
    try {
      ...
      try {
        ...
        const list_id = rows.map(({ product_id, similar_product_id }) => {
          return origin_product === product_id
            ? similar_product_id
            : product_id;
        });
...
productsRouter.post(
  ...
  [
    ...
    body("*.similar_product_id")
      .isUUID()
      .withMessage("Similar products id is not UUID"),
  ],
    ...
    try {
      ...
      try {
        const [uniquenessСheck] = await connection.query<ResultSetHeader>(
          `SELECT * FROM similar_products WHERE product_id IN (?)`,
          [req.body.product_id]
        );
        ...
        const [rows] = await Promise.all(
          req.body.map(async (product) => {
            const [info] = await connection.query<ResultSetHeader>(
              postSimilarProducts,
              [product.product_id, product.similar_product_id]
            );
            return info;
          })
        );
...
productsRouter.post(
  "/del_similar_products",
  [
    body().isArray().withMessage("Body should be an array"),
    body("*.product_id")
      .isUUID()
      .withMessage("Each product_id should be a valid UUID"),
    body("*.similar_product_id")
      .isUUID()
      .withMessage("Each similar_product_id should be a valid UUID"),
  ],
  async (req: Request, res: Response) => {
    try {
      const errors = validationResult(req);
      ...
      try {
        const [resultsQuerys] = await Promise.all(
          req.body.map(async (product) => {
            const [info] = await connection.query<ResultSetHeader>(
              delleteSimilarProducts,
              [product.product_id, product.similar_product_id]
