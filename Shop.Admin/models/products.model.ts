...
export async function updateProduct(
  ...
  try {
    ...
    if (formData.commentsToRemove) {
      try {
        const commentsIdsToRemove = compileIdsToRemove(
          formData.commentsToRemove
        );

        await Promise.all(
          commentsIdsToRemove.map(async (id) => {
    ...
    if (formData.imagesToRemove) {
      try {
        ...
        await Promise.all(
          imagesIdsToRemove.map(async (id) => {
    ...
    if (formData.newImages) {
      try {
        const imagesToAdd = await compileImagesToAdd(
          formData.newImages,
          currentProduct
        );
    ...
    if (
      ...
      try {
        ...
        await axios.post(
          `${host}/products/update_thumbnail/${currentProduct.id}`,
          data
        );
      } catch (e) {
        console.log(e);
    ...
    if (formData.similarProductToRemove) {
      ...
      for (let products of ids) {
        if (!removingLinks.has(currentProduct.id)) {
          removingLinks.set(currentProduct.id, {
            product_id: currentProduct.id,
            similar_product_id: products,
          });
        }
        ...
        removingLinks.set(currentProduct.id, [
          ...list,
          { product_id: currentProduct.id, similar_product_id: products },
        ]);
      }

      await axios.post(
        `${host}/products/del_similar_products`,
        removingLinks.get(currentProduct.id)
      );
    }
    ...
    if (formData.add_similar_product) {
      ...
      for (let products of pairs) {
        if (!addLinks.has(currentProduct.id)) {
          addLinks.set(currentProduct.id, {
            product_id: currentProduct.id,
            similar_product_id: products,
          });
        }
      ...
      await axios.post(
        `${host}/products/add_similar_products`,
        addLinks.get(currentProduct.id)
      );
    }
